  ...
  <div class="flex min-h-dvh flex-col items-center justify-center bg-gray-50 px-4 py-10">
    <div class="w-full max-w-2xl">
      ...
      <div class="rounded border border-gray-200 bg-white">
        <div class="flex items-center justify-between border-b border-gray-100 px-5 py-4">
          <div>
            <h1 class="text-sm font-bold text-gray-900">
              {{ step === 'profile' ? 'Agent profile setup' : 'Upload verification documents' }}
            </h1>
            <p class="mt-0.5 text-xs text-gray-500">
              <template v-if="step === 'profile'">
                This is what landlords will see when reviewing your listing requests.
                You can refine it any time after onboarding.
              </template>
              <template v-else>
                Optional but recommended. Upload your license, ID, and any portfolio
                references. CribHub will verify your account based on these documents.
              </template>
            </p>
          </div>
          <NuxtLink
        ...
        <!-- Step 1: profile form -->
        <form v-if="step === 'profile'" class="space-y-5 p-5" @submit.prevent="onSubmit">
          <section>
            ...
            <div class="grid gap-3 sm:grid-cols-2">
              <div>
                ...
                <input
                  ...
                  class="field-input"
                />
                <p class="mt-0.5 text-[11px] text-gray-400">Shown on every listing and request.</p>
              </div>
              <div>
                <label class="field-label">Agency / brokerage</label>
                <input
                  v-model="form.agency"
                  type="text"
                  placeholder="e.g. Urban Living Realty"
                  class="field-input"
                />
              </div>
              <div>
                <label class="field-label">License number <span class="text-red-500">*</span></label>
                <input
          ...
          <section>
            ...
            <div class="grid gap-3 sm:grid-cols-3">
              <div>
                ...
                <input v-model="form.phone" type="tel" placeholder="+233 ..." class="field-input" />
              </div>
              <div>
          ...
          <section>
            <h2 class="mb-2 flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider text-gray-500">
              <i class="las la-align-left text-sm text-gray-400"></i> About you
            </h2>
            <textarea
              v-model="form.bio"
              rows="3"
          ...
          <section>
            <h2 class="mb-2 flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wider text-gray-500">
              ...
              <span class="ml-1 text-[10px] font-normal normal-case text-gray-400">
                (optional · pre-fills your fee on every listing request)
              </span>
            </h2>
        ...
        <div v-else class="space-y-3 p-5">
          ...
          <p class="text-[11px] text-gray-400">
            Accepted: license, ID, business registration, portfolio references. PDF, DOC, or images up to 10 MB each.
          </p>
        </div>
...
import { useAgentProfilesStore } from '@@/stores/operations'
...
async function onSubmit() {
  ...
  try {
    ...
    const payload: Record<string, any> = {
      ...
      yearsExperience: form.years ?? undefined,
    }
    ...
    await agentProfiles.fetchList({ userId: id })
    const existing = (agentProfiles.list as any[]).find((p) => String(p.userId) === String(id))
    let saved: any
    if (existing?._id) {
      saved = await agentProfiles.patch(String(existing._id), payload)
    } else {
      saved = await agentProfiles.create(payload)
    }
